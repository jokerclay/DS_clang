...
**realloc 方法是如何进行扩容的**
realloc 会试着 在 指针`p` 后面申请一段空间
如果能成功的在原来的内存后面申请一段空间，返回的就是原来的 p 指针
如果没有办法在之后申请一段, `realloc` 会调用 `malloc` 重新分配一段空间 `p'`
申请到新的数据之后，会把原空间中的数据拷贝过来,拷贝过来之后,再返回新申请的空间的首地址
再将原来的内存空间 `p` 释放 free
如果申请新的地址空间都失败了的话，会返回 空指针 `NULL` 代表重新分配地址空间的操作失败(原来的内存空间不动)
